...
use common::{
    definitions::{Arm, Command, DriverType, Position, Status},
    error::HardwareError::{self, I2cSetSlave, *},
};
...
impl TemplateApp {
    ...
    pub fn connect(url: &str) -> EventClient {
        ...
        client.set_on_message(Some(Box::new(|_, message: Message| {
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::TOP), |ui| {
            ui.menu_button("Commandes", |ui| {
                ...
                if ui.button("Reset").clicked() {
                    self.reset_driver(if is_emitter {
                        DriverType::E
                    } else {
                        DriverType::R
                    });
    ...
    pub fn other_side_panel(&mut self, ui: &mut egui::Ui, is_emitter: bool) {
        ...
        ui.add_enabled_ui(change, |ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .max_height(if is_emitter {
                    ui.available_height() / 2.0 - 50.0
                } else {
                    ui.available_height()
                })
    ...
    pub fn main_view(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ui.vertical_centered(|ui| {
            ...
            let depth = (ui.available_height() - 20.0).min(width * 680.0 / 1417.0);
    ...
    fn get_new_frame(
        ...
        let arm = if is_left { &self.left } else { &self.right };

        let mut next_pos_pos = if is_left { self.next_e } else { self.next_r };
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ...
                let pos = ui.min_rect().min
                    ...
                    + egui::vec2(
                        (arm.position().x() + if is_left { 1417.0 } else { -70.0 }) * width
                            / 1347.0,
                        if is_up {
                ...
                egui::Image::new(egui::include_image!("../assets/emitter.png"))
                    .max_size(egui::vec2(30.0, 30.0))
                    .rotate(
                        if is_up {
                            arm.position().theta() * PI / 180.0
                                + if is_left { PI / 2.0 } else { -PI / 2.0 }
                        } else {
                ...
                egui::Image::new(egui::include_image!("../assets/emitter.png"))
                    .max_size(egui::vec2(30.0, 30.0))
                    .tint(Color32::from_rgba_premultiplied(0, 0, 0, 100))
                    .rotate(
                        if is_up {
                            next_pos_pos.theta() * PI / 180.0
                                + (PI / 2.0 * if is_left { 1.0 } else { -1.0 })
                        } else {
                ...
                if area.dragged() {
                    let mut pix_pos = area.rect.min + egui::vec2(5.0, 5.0)
                        - ui.min_rect().min
                        - egui::vec2(if is_left { 25.0 } else { 5.0 }, 25.0)
                        + area.drag_delta();
                ...
                for (i, pos) in arm.list_next().iter().enumerate() {
                    ...
                    let couleur = Color32::from_rgba_premultiplied(
                        if i == 0 && !DRIVER_USED.lock().unwrap().borrow().is_empty() {
                            128
                        } else {
                            0
                        },
                    ...
                    ui.painter().circle_filled(list_pos, 2.5, couleur);

                    ui.painter()
                        .line_segment([prev_pos, list_pos], egui::Stroke::new(1.0, couleur));
            ...
            match self.file_dialog.1.try_recv() {
                Ok(string) => {
                    ...
                    match serde_json::from_str::<(Vec<Position>, Vec<Position>)>(string.as_str()) {
                        ...
                        Err(_) => ERR_LIST.lock().unwrap().push(HardwareError::UnknownError(
                            "Format de fichier invalide".to_string(),
                        )),
                    }
                }
                Err(_) => {
    ...
    pub fn send(&mut self, data: Command) {
        let msg = serde_json::to_string(&data).expect("JSON conversion error");
                ...
                ui.menu_button("Fichier", |ui| {
                    if ui.button("Sauvegarder").clicked() {
                        ...
                        let data = serde_json::to_string(&(
                            self.left.list_next().clone(),
                            self.right.list_next().clone(),
                        ))
                    ...
                    if ui.button("Charger").clicked() {
                        ...
                        execute(async move {
                            ...
                            if let Some(file) = file {
                                ...
                                let _ =
                                    message_sender.send(String::from_utf8_lossy(&text).to_string());
                            }
                        });
                    }
                });
