...
use crate::error_handler::{write_error_log, write_tcp_log};

/// Mutex du stream de la connexion TCP
pub static STREAM: Mutex<RefCell<Option<WebSocket<TcpStream>>>> = Mutex::new(RefCell::new(None));
        ...
        while match MUTEX_USED.try_lock() {
            Ok(used) => {
                if used.borrow().eq(&false) {
                    ...
                    match borow_stream.as_mut().unwrap().read() {
                        Ok(Message::Text(msg)) => {
                            let command: Command;
                            match serde_json::from_str(msg.as_str()) {
                                Ok(json) => {
                                    write_tcp_log(format!(
                                        "Data({:?}) received in  Thread Update",
                                        json
                                    ));
                                    command = json;
                                    ...
                                    let update_result = vec![DRIVERS
                                        ...
                                        .unwrap()
                                        .update(command)];
                                    let result = serde_json::to_string(&if ERR_LIST
                                        .lock()
                                        .unwrap()
                                        .borrow()
                                        .is_empty()
                                        ...
                                        .send(Message::Text(result.clone().into()))
                                    {
                                        ...
                                        Err(_) => {
                                            write_error_log(format!(
                                                "Could not send data({:?}) from Thread Update",
                                                result
                                            ));
                                            true
                                        }
                                ...
                                Err(_) => {
                                    write_error_log(format!(
                                        "Unrecognizable data({}) received in Thread Update",
                                        msg
                                    ));
                                }
                            }
...
fn handle_client() -> std::io::Result<()> {
    let _join_1 = match Builder::new()
        .name("update_thread".to_string())
        .spawn(|| update_thread_function())
    {
        Ok(jh) => jh,
        Err(e) => {
    ...
    let _join_2 = match Builder::new()
        .name("status_theard".to_string())
        .spawn(|| status_thread_function())
    {
        Ok(jh) => jh,
        Err(e) => {
...
pub fn tcp_listen() -> std::io::Result<()> {
    STREAM_LOG_ERRORS.lock().unwrap().replace(Some(
        OpenOptions::new() /*.write(true)*/
            .append(true)
            .create(true)
            .open("./log/error.log")
            .expect("Erreur ouverture fichier ./log/error.log"),
    ));
        ...
        match stream {
            Ok(tcp_stream) => {
                let websocket = match accept(tcp_stream) {
                    Ok(ws) => ws,
                    Err(e) => {
                        write_error_log(format!("WebSocket handshake failed: {:?}", e));
                        continue;
                    }
                };

                let peer_addr = match websocket.get_ref().peer_addr() {
                    Ok(addr) => addr,
                    Err(e) => {
                        write_error_log(format!("Could not get peer address: {:?}", e));
                        continue;
                    }
                };

                // Set non-blocking mode
                match websocket.get_ref().set_nonblocking(true) {
                    Ok(_) => {}
                    Err(e) => {
                ...
                let _ = STREAM.lock().unwrap().replace(Some(websocket));

                write_tcp_log(format!("New connection: {}", peer_addr));
                handle_client()?;
            }
            Err(_) => {
